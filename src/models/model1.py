...
class Model1(BaseModel):
    ...
    def _init_set_input_params(self):
        ...
        self._Cd = self._opt[self._dataset_type]["depth_nc"]                # num channels depth
    ...
    def forward(self, keep_data_for_visuals=False, estimate_loss=True):
        # generate img
        estim = self._estimate(self._input_img, self._input_depth)
    ...
    def _estimate(self, img, depth):
        return self._reg.forward(img, depth)

    def _keep_data(self, estim):
        self._vis_img = util.tensor2im(self._input_img.detach().clone(), unnormalize=True)
        self._vis_depth = util.tensor2im(self._input_depth.detach().clone(), unnormalize=True)
        self._vis_input_img = plot_estim(util.tensor2im(self._input_img.detach().clone(), to_numpy=True),
                                         estim.max(1)[1].detach().cpu().numpy(),
                                         self._input_target.detach().cpu().numpy())
    ...
    def get_current_visuals(self):
        visuals = OrderedDict()
        visuals["1_input_img"] = self._vis_img
        visuals["2_input_depth"] = self._vis_depth
        visuals["3_estim"] = self._vis_input_img
        return visuals
